    ...
    void checkDataIR(uint8_t incomingByte) {
      switch (currentStateIR) {
        ...
        case READ_CHECKSUM_IR:
          currentStateIR = (checksumIR == incomingByte) ? WAIT_FOR_END_IR
                                                        : WAIT_FOR_START_IR;
          break;
    ...
    void checkDataLS(uint8_t incomingByte) {
      switch (currentStateLS) {
        ...
        case READ_ANGLE_LOW_LS:
          ...
          checksumLS   += incomingByte;
          currentStateLS = READ_ERR_LS;
          break;

        case READ_ERR_LS:
          localErr       = incomingByte;
          currentStateLS = READ_CHECKSUM_LS;
          break;

        case READ_CHECKSUM_LS:
          currentStateLS = (checksumLS == incomingByte) ? WAIT_FOR_END_LS
                                                        : WAIT_FOR_START_LS;
          break;
        ...
        case WAIT_FOR_END_LS:
          if (incomingByte == 254) {
            ...
            if (angleLS != 500) angleLS -= 180;
            lsErr   = (localErr > 100) ? (localErr - 200) : localErr;
          }
          currentStateLS = WAIT_FOR_START_LS;
          break;
      }
